...
# ---------------------------------------------
# 将 Fusion 产物同步到 firmware（供静态页同源加载）
# ---------------------------------------------
copy_fusion_firmware_to_docs() {
  local src="build/Fusion/f4hwn.fusion.bin"
  if [[ -f "$src" ]]; then
    mkdir -p docs
    cp -f "$src" docs/f4hwn.fusion.bin
    echo "📁 已复制固件到 docs/f4hwn.fusion.bin"
  else
    echo "⚠️ 未找到 $src，跳过复制到 docs"
  fi
}
# ---------------------------------------------
# Handle 'All' preset
