...
2. 默认启用BYP模式
3. 尾音去掉MDC，改为滴滴 730Hzx2
4. 收音机状态停止接收信道信号后，返回收音机状态由5s改为1s；双守状态下某个信道信号结束后4.2秒返回双守状态改为2.1s
5. 启用开机bibi声
6. 侧键修改的rxmode，保存，侧键仅切换MO和DW
7. 锁定发射的频率，不显示频差方向和发射功率
8. 禁用tx、rx计时
9. F8关闭常亮，F9同长按9
10. 减小对讲机音量，匹配收音机
11. 630-840可接收
12. 消除频率模式下切换频率的咔咔声
13. 取消收音机模式下的按键音，影响音频
14. 收音机FM状态栏指示
15. RXmode状态图标修改
16. 长按5直接进入频谱图
17. 取消 * 短按：进入DTMF输入模式
...
本工具是适配泉盛 UV-K1 对讲机的专用字体烧写工具，兼容 F4HWN 开源固件，支持 8×8、16×16 两种点阵字体的自定义生成与烧录，可替换设备系统界面显示字体，完整覆盖字体制作、固件适配、字体烧录全套操作流程。
字库存储地址可直接寻址调用

    16×16 大号点阵字库：0xA0000 ~ 0xDFE3F
    8×8 小号点阵字库：0xE0000 ~ 0xEFF8F

以上两段 Flash 地址区域未被原厂固件占用，玩家可按需自制专属自定义字体。
软件架构

Windows端可执行程序，独立工具，与F4HWN对讲机固件源码相互独立，无代码耦合，不受固件GPL协议约束，可自主选用宽松开源协议分发。
安装教程

    直接下载仓库内文件，无需额外安装依赖，解压后即可运行 UV-K1_FontTool.exe
    配套字体模板、字体专用过渡固件已同步存放于仓库目录内，开箱即用
    字体生成参数参考图文件一同打包在仓库，制作自定义字体时对照使用

使用说明
UV-K1 自定义字体完整操作流程
进行操作前你需要一根K头的写频线，USB无法正常工作

    一定先导出校准数据，刷机等操作都可以在如下地址进行，https://armel.github.io/uvtools2/
    先刷入目录内官方编译开源固件，体验并确认界面风格是否符合自身需求
    配套字体文件已存放至对应目录，直接取用即可
    如需自制自定义字体，对照配图参数设置字体格式
    小字体其余参数和大字体保持一致，仅点阵尺寸改为8×8
    依照图片操作指引，一键生成点阵字体文件
    生成后的两个字体文件分别命名：16x16.fon、8x8.fon
    刷入目录内提供的字体专用固件
    打开UV-K1_FontTool.exe导入字体刷写，单次仅刷入一个字体，刷完关机后再刷第二个
    全部字体刷写完成，最后刷入集成自定义字体的定制版固件
